...
from other_code.train_original import DataGenerator
...
if __name__ =='__main__':
    ...
    parser.add_argument('--image-path', required=True, nargs="+", help="*.jpg file to generate predictions for. Can be a glob.")
    args = parser.parse_args(sys.argv[1:])

    df = pd.read_pickle(args.dataframe)
    model = load_model(args.weight_file)
    data_encoder = DataGenerator(args.data_directory, df).encoder
